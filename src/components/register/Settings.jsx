...
export default function Settings({ currentStep, handlePrevious }) {
  ...
  const schemaBuilder = (
    ...
    const baseSchema = yup.object().shape({
      ...
      countriesCurrency: requiredValidatorOfArrayNew(
        "Operating countries and currencies"
      ),
      informationMedium: requiredValidatorOfArrayNew(
    ...
    if (shipperModule.includes("Tracking")) {
      conditionalSchema = conditionalSchema.shape({
        ...
        pinTracking: requiredValidator("Tracking pin duration"),
        ...
        tripClosureRadius: requiredNonZeroPositiveValidator(
          "Trip closure radius"
        ),
    ...
    if (shipperModule.includes("Bidding")) {
      conditionalSchema = conditionalSchema.shape({
        ...
        bidIncrement: requiredNonZeroPositiveValidator(
          "Bid increment time to be applied at"
        ),
        bidPriceDecrement: requiredNonZeroPositiveValidator(
          "Bid price decrement"
        ),
        bidLimit: requiredNonZeroPositiveValidator("No. of Bids"),
        bidMatching: requiredValidator("Bid matching"),
        // bidMatchingDuration: requiredNonZeroPositiveValidator(
        //   'Bid matching duration'
        // ),
        currentLowestRate: requiredValidator(
          "Cureent lowest rate to transporter"
        ),
        showContactDetailsToTransporter: requiredValidator(
          "Show ontact details to transporter"
        ),
    ...
    if (selectedAlertBox && selectedAlertBox.length > 0) {
      ...
      selectedAlertBox.forEach((alertOption) => {
        ...
        if (alertOption !== "E-way bill") {
          schema.highAlertTime =
            requiredNonZeroPositiveValidator("High alert time");
          schema.lowAlertTime =
            requiredNonZeroPositiveValidator("Low alert time");
          alertBoxSchema[`${alertOption}Alert`] = yup.object().shape(schema);
        }
      });
      alertBoxSchema.notificationChannels = requiredValidatorOfArrayNew(
        "Notification channel is required"
      );
      alertBoxSchema.alertRecipents = requiredValidatorOfArrayNew(
  ...
  const fetchShipperModule = async () => {
    setIsLoading(true);
    return getShipperModule(shipperId)
      .then((res) => {
  ...
  const updateBasicSettings = (basicData) => {
    ...
    setValue(
      "pinTracking",
      basicData.tracking_ping === 15 || basicData.tracking_ping === 30
        ? basicData.tracking_ping
        : "custom"
    );
    setCustomTracking(
      !(basicData.tracking_ping === 15 || basicData.tracking_ping === 30)
  ...
  const updateBidSettings = (bidData) => {
    ...
    setValue(
      "currentLowestRate",
      bidData.show_current_lowest_rate_transporter ? "Yes" : "No"
    );
    setValue(
      "showContactDetailsToTransporter",
      bidData.show_contact_details_to_transporter ? "Yes" : "No"
    );
    setValue(
      "rateQuationType",
      bidData.bdsttng_rate_quote_type === "PMT" ||
        bidData.bdsttng_rate_quote_type === "FTL"
        ? bidData.bdsttng_rate_quote_type
        : "custom"
    );
    setRateCustom(
      !(
  ...
  const updateFormValues = (alertData) => {
    ...
    setValue(
      "alertRecipents",
      alertData.alert_recipents
        ? arrData.length > 0 && alertData.alert_recipents.split(", ")
        : []
    );
    setValue(
      "HoldAlert.lowAlertTime",
      alertData.hold_alert_enabled ? alertData.vehicle_hold_short : ""
    );
    setValue(
      "HoldAlert.highAlertTime",
      alertData.hold_alert_enabled ? alertData.vehicle_hold_long : ""
    );
    setValue(
      "DelayAlert.lowAlertTime",
      alertData.delay_alert_enabled ? alertData.delay_short : ""
    );
    setValue(
      "DelayAlert.highAlertTime",
      alertData.delay_alert_enabled ? alertData.delay_long : ""
    );
    setValue(
      "ArrivalAlert.lowAlertTime",
      alertData.arrival_alert_enabled ? alertData.arrival_short : ""
    );
    setValue(
      "ArrivalAlert.highAlertTime",
      alertData.arrival_alert_enabled ? alertData.arrival_long : ""
    );
    setValue(
      "DepartureAlert.lowAlertTime",
      alertData.departure_alert_enabled ? alertData.departure_short : ""
    );
    setValue(
      "DepartureAlert.highAlertTime",
      alertData.departure_alert_enabled ? alertData.departure_long : ""
    );
    setValue(
      "DeviationAlert.lowAlertTime",
      alertData.route_deviate_alert_enabled ? alertData.route_deviate_short : ""
    );
    setValue(
      "DeviationAlert.highAlertTime",
      alertData.route_deviate_alert_enabled ? alertData.route_deviate_long : ""
    );
    setValue(
      "SOSAlert.lowAlertTime",
      alertData.sos_alert_enabled ? alertData.sos_short : ""
    );
    setValue(
      "SOSAlert.highAlertTime",
      alertData.sos_alert_enabled ? alertData.sos_long : ""
    );
  };

  const setSelectedCurrencies = (currencies) => {
    const newArray = currenciesData.filter((obj) =>
      currencies.includes(obj.value)
    );
  ...
  const onSubmit = (data) => {
    ...
    if (flag) {
      selectedAlertBox.forEach((alertSetting) => {
        if (alertSetting !== "E-way bill") {
          if (
            data[`${alertSetting}Alert`].lowAlertTime >=
            data[`${alertSetting}Alert`].highAlertTime
          ) {
            setError(`${alertSetting}Alert.highAlertTime`, {
              message: "High alert time must be large than low alert time",
            });
            flag = false;
          }
        }
    ...
    if (flag) {
      if (data.bidIncrement >= data.bidDuration) {
        setError("bidIncrement", {
          message:
            "Bid increment time to be applied at must be less than bid duration",
        });
        flag = false;
      }
    }
    if (flag) {
      if (data.arrivalRadius < data.tripClosureRadius) {
        setError("tripClosureRadius", {
          message: "Trip closure radius must be less than arrival radius",
        });
        flag = false;
      }
    }
    if (data.bidMatching === "Yes" && flag) {
      if (data.bidMatchingDuration === "") {
        setError("bidMatchingDuration", {
          message: "Bid matching duration is required and must be a number",
        });
        flag = false;
      } else if (data.bidMatchingDuration < 0) {
        setError("bidMatchingDuration", {
          message: "Bid matching duration must be a positive number",
        });
        flag = false;
      }
    }
    if (flag) {
      const payload = {
        ...
        basic: {
          ...
          tracking_ping:
            data.pinTracking === "custom"
              ? parseInt(data.customPin)
              : parseInt(data.pinTracking),
        ...
        bid: {
          ...
          bid_duration:
            data.bidDuration && data.bidDuration !== ""
              ? parseInt(data.bidDuration)
              : 0,
          bid_increment_time:
            data.bidDecrement && data.bidDecrement !== ""
              ? parseInt(data.bidDecrement)
              : 0,
          bid_increment_duration:
            data.bidIncrement && data.bidIncrement !== ""
              ? parseInt(data.bidIncrement)
              : 0,
          bid_price_decrement:
            data.bidPriceDecrement && data.bidPriceDecrement !== ""
              ? parseInt(data.bidPriceDecrement)
              : 0,
          ...
          price_match_duration:
            data.bidMatchingDuration &&
              data.bidMatchingDuration !== "" &&
              data.bidMatching === "Yes"
              ? parseInt(data.bidMatchingDuration)
              : 0,
          show_current_lowest_rate_transporter:
            data.currentLowestRate === "Yes",
          show_contact_details_to_transporter:
            data.showContactDetailsToTransporter === "Yes",
          bdsttng_rate_quote_type: data.rateQuationType
            ? data.rateQuationType === "custom"
              ? data.customQuationType
              : data.rateQuationType
            : "",
        },
        alert: {
          ...
          alert_recipents:
            selectedAlertBox.length > 0
              ? arrayJoinFunction(data.alertRecipents)
              : null,
          notification_channels:
            (shipperModule === "Bidding & Tracking" || shipperModule === "Tracking") &&
              selectedAlertBox.length > 0
              ? arrayJoinFunction(data.notificationChannels)
              : null,
          vehicle_hold_long: selectedAlertBox.includes("Hold")
            ? data[`HoldAlert`].highAlertTime &&
            parseInt(data[`HoldAlert`]?.highAlertTime)
            : 0,
          vehicle_hold_short: selectedAlertBox.includes("Hold")
            ? data[`HoldAlert`].lowAlertTime &&
            parseInt(data[`HoldAlert`]?.lowAlertTime)
            : 0,
          delay_long: (shipperModule === "Bidding & Tracking" || shipperModule === "Tracking") && selectedAlertBox.includes("Delay")
            ? data[`DelayAlert`].highAlertTime &&
            parseInt(data[`DelayAlert`]?.highAlertTime)
            : 0,
          delay_short: (shipperModule === "Bidding & Tracking" || shipperModule === "Tracking") && selectedAlertBox.includes("Delay")
            ? data[`DelayAlert`].lowAlertTime &&
            parseInt(data[`DelayAlert`]?.lowAlertTime)
            : 0,
          arrival_long: selectedAlertBox.includes("Arrival")
            ? data[`ArrivalAlert`].highAlertTime &&
            parseInt(data[`ArrivalAlert`]?.highAlertTime)
            : 0,
          arrival_short: selectedAlertBox.includes("Arrival")
            ? data[`ArrivalAlert`].lowAlertTime &&
            parseInt(data[`ArrivalAlert`]?.lowAlertTime)
            : 0,
          departure_long: selectedAlertBox.includes("Departure")
            ? data[`DepartureAlert`].highAlertTime &&
            parseInt(data[`DepartureAlert`]?.highAlertTime)
            : 0,
          departure_short: selectedAlertBox.includes("Departure")
            ? data[`DepartureAlert`].lowAlertTime &&
            parseInt(data[`DepartureAlert`]?.lowAlertTime)
            : 0,
          route_deviate_long: selectedAlertBox.includes("Deviation")
            ? data[`DeviationAlert`].highAlertTime &&
            parseInt(data[`DeviationAlert`]?.highAlertTime)
            : 0,
          route_deviate_short: selectedAlertBox.includes("Deviation")
            ? data[`DeviationAlert`].lowAlertTime &&
            parseInt(data[`DeviationAlert`]?.lowAlertTime)
            : 0,
          sos_long: selectedAlertBox.includes("SOS")
            ? data[`SOSAlert`].highAlertTime &&
            parseInt(data[`SOSAlert`]?.highAlertTime)
            : 0,
          sos_short: selectedAlertBox.includes("SOS")
            ? data[`SOSAlert`].lowAlertTime &&
            parseInt(data[`SOSAlert`]?.lowAlertTime)
            : 0,
        },
        geofence: {
          gfnc_sttng_id: geofenceSttngId === "" ? null : geofenceSttngId,
          source_radius:
            data.sourceRadius && data.sourceRadius !== ""
              ? parseInt(data.sourceRadius)
              : 0,
          arrival_radius:
            data.arrivalRadius && data.arrivalRadius !== ""
              ? parseInt(data.arrivalRadius)
              : 0,
          trip_close_radius:
            data.tripClosureRadius && data.tripClosureRadius !== ""
              ? parseInt(data.tripClosureRadius)
              : 0,
          trip_close_method: data.rateQuationTypeRadio
            ? data.rateQuationTypeRadio
            : "none",
        },
        comments: [],
      };
    ...
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <ContentWrapper>
                        ...
                        render={({ field }) => (
                          <Grid container spacing={2}>
                            <Grid item xs={12} sm={6}>
                              <div
                                className={`radio-option ${field.value?.includes("contractual")
                                  ? "selected"
                                  : ""
                                  }`}
                                onClick={() => {
                            ...
                            <Grid item xs={12} sm={6}>
                              <div
                                className={`radio-option ${field.value?.includes("spot")
                                  ? "selected"
                                  : ""
                                  }`}
                                onClick={() => {
                              ...
                              >
                                <div className="checkBoxIcon">
                                  <Checkbox
                                    checked={field.value?.includes("spot")}
                                  />
                                </div>
                                <div className="radiotext">
                      ...
                      <Grid item xs={12} sm={6} md={4}>
                        ...
                        >
                          <div className="samllcheckBoxIcon">
                            <Checkbox checked={field.value?.includes("sms")} />
                          </div>
          ...
          {shipperModule === "Tracking" ||
            shipperModule === "Bidding & Tracking" ? (
            <RegisterCard>
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                  <Grid container spacing={2}>
                          ...
                          endAdornment: (
                            ...
                            >
                              <Select
                                ...
                                label="Age"
                                onChange={(e) =>
                                  setPriceDecrementType(e.target.value)
                                }
                              >
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <Grid container spacing={2}>
                            <Grid item xs={12} sm={6}>
                              <div
                                className={`radio-option ${field.value === "private_pool"
                                  ? "selected"
                                  : ""
                                  }`}
                            ...
                            <Grid item xs={12} sm={6}>
                              <div
                                className={`radio-option ${field.value === "open_market"
                                  ? "selected"
                                  : ""
                                  }`}
            ...
            <RegisterCard>
              <Grid container spacing={2}>
                <Grid item xs={12}>
                  ...
                  <Typography variant="p">
                    Select single or multiple alerts you want for your
                    organization
                  </Typography>
                </Grid>
                <Grid item xs={12}>
                      ...
                      <Grid container spacing={2}>
                        {alertCheckboxOptions.map((alertBox) => (
                          <Grid item xs={12} sm={6} md={4} key={alertBox}>
                            <div
                              className={`radio-option ${field.value?.includes(alertBox)
                                ? "selected"
                                : ""
                                }`}
                              onClick={() => {
                            ...
                            >
                              <div className="checkBoxIcon">
                                <Checkbox
                                  checked={field.value?.includes(alertBox)}
                                />
                              </div>
                              <div className="radiotext">
                                <Typography variant="h5">
                ...
                {selectedAlertBox.length !== 0 ? (
                  <Grid item xs={12}>
                    <Grid container spacing={2}>
                              ...
                              <Controller
                                ...
                                render={({ field }) => (
                                  <Grid container spacing={2}>
                                    <Grid item xs={12} sm={6} md={3}>
                                      <div
                                        className={`checkbox-option ${field.value?.includes("sms")
                                          ? "selected"
                                          : ""
                                          }`}
                                        onClick={() => {
                                          const updatedValue =
                                            field.value?.includes("sms")
                                              ? field.value.filter(
                                                (item) => item !== "sms"
                                              )
                                              : [...field.value, "sms"];
                              ...
                              <Controller
                                ...
                                render={({ field }) => (
                                  <Grid container spacing={2}>
                                    <Grid item xs={12} sm={6} md={4}>
                                      <div
                                        className={`radio-option ${field.value?.includes("shipper")
                                          ? "selected"
                                          : ""
                                          }`}
                                        onClick={() => {
                                          const updatedValue =
                                            field.value?.includes("shipper")
                                              ? field.value.filter(
                                                (item) => item !== "shipper"
                                              )
                                              : [...field.value, "shipper"];
                                    ...
                                    <Grid item xs={12} sm={6} md={4}>
                                      <div
                                        ...
                                        onClick={() => {
                                          const updatedValue =
                                            field.value?.includes("transporter")
                                              ? field.value.filter(
                                                (item) =>
                                                  item !== "transporter"
                                              )
                      ...
                      {(!selectedAlertBox.includes("E-way bill") ||
                        selectedAlertBox.length > 1) && (
                          <Grid item xs={12}>
                            <Typography variant="h4">
                          ...
                          alertSetting !== "E-way bill" && (
                            <Grid item key={alertSetting} xs={12}>
                              <Accordion
                                ...
                                onChange={() =>
                                  handleCheckboxClick(alertSetting)
                                }
                              >
                                <AccordionSummary
                                      ...
                                      <Controller
                                        ...
                                        render={({ field }) => (
                                          <TextField
                                            ...
                                            error={Boolean(
                                              errors[`${alertSetting}Alert`]
                                                ?.lowAlertTime
                                            )}
                                            helperText={
                                              errors[`${alertSetting}Alert`]
                                                ?.lowAlertTime?.message
                                            }
                                            sx={{
